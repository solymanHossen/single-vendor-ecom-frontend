...
export async function getHeroBanners(placement?: HeroBannerPlacement): Promise<HeroBanner[]> {
...
export async function getAllHeroBannersAdmin(accessToken: string): Promise<HeroBanner[]> {
...
export async function deleteHeroBanner(accessToken: string, id: number): Promise<void> {
  const response = await backendFetch(`/hero-banners/${id}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${accessToken}` },
  });
